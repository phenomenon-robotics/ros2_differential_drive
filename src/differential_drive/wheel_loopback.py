...
class WheelLoopback(Node):
    ...
    def __init__(self):
        ...
        self.rate = self.declare_parameter("rate", 200)
        self.timeout_secs = self.declare_parameter("timeout_secs", 0.5)
        self.ticks_meter = float(self.declare_parameter('ticks_meter', 50))
        self.velocity_scale = float(self.declare_parameter('velocity_scale', 255))
        self.latest_motor = 0

        self.wheel = 0

        self.create_subscriber(Int16, 'motor', self.motor_callback)

        self.pub_wheel = self.create_publisher(Int16, 'wheel', queue_size=10)

    def spin(self):
        r = rclpy.Rate
        self.secs_since_target = self.timeout_secs
        self.then = self.get_clock().now()
        self.latest_msg_time = rclpy.get_clock().now()
        rclpy.get_logger().info("-D- spinning")
        ...
        while rclpy.ok():
            while rclpy.ok() and self.secs_since_target < self.timeout_secs:
                self.spin_once()
                r.sleep
                self.secs_since_target = rclpy.get_clock().now() - self.latest_msg_time
            ...
            self.secs_since_target = rclpy.Time.now() - self.latest_msg_time
            self.secs_since_target = self.secs_since_target.to_sec()
    ...
    def spin_once(self):
        ...
        if abs(self.velocity) > 0:
            ...
            if elapsed > self.seconds_per_tick:
                rclpy.get_logger().info("incrementing wheel")
                if self.velocity > 0:
                    self.wheel += 1
                else:
                    self.wheel -= 1
                self.pub_wheel.publish(self.wheel)
                self.then = rclpy.get_clock().now()
